    ...
    <div className='bg-slate-200 p-2 flex gap-4 rounded border-2 border-slate-300'>
        ...
        <div className='flex flex-col gap-1 w-full'>
          <div className='flex justify-between'>
           ...
           <p className=' font-bold text-base'><span className='text-red-500 '>{name?"₹":"Loading"}</span><span>{price}</span></p>
           <div className='flex justify-between '>
           <div className='flex gap-3 items-center'>
           <button className='bg-slate-300 py-1 mt-1 mb-2 rounded hover:bg-slate-600 p-1' onClick={()=>dispatch(increaseQty(id))}><TbPlus/></button>
           <p className='font-semibold p-1'>{qty}</p>
           <button className='bg-slate-300 py-1 mt-1 mb-2 rounded hover:bg-slate-600 p-1' onClick={()=>dispatch(decreaseQty(id))}><TbMinus/></button>
           </div>
